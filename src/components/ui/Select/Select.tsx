    ...
    return (
        <Listbox value={selectedValue} onChange={handleSelectChange}>
            <div className={root({ size, class: `relative ${className}` })}>
                <Listbox.Button className={`inputBox w-full h-full border rounded-md py-3 px-2 flex items-center relative ${selectedValue ? 'bg-background-base' : 'text-foreground'} border-2 border-background-base hover:border-foreground focus:border-primary focus:outline-primary`}>
                    ...
                    <span className="absolute right-2 top-1/2 transform -translate-y-1/2"><MdKeyboardArrowDown size={22} /></span>
                ...
                >
                    <Listbox.Options className="absolute mt-2 -left-[2rem] origin-top bg-background-base text-foreground rounded-md shadow-lg focus:outline-none sm:text-sm w-[250px]">
                        {options.map((option, index) => (
                            <Listbox.Option key={index} value={option.value} className="">
                                {({ active, selected }) => (
                                    <div
                                        className={`w-full h-full py-3 px-4 flex items-center ${active ? "text-white bg-primary" : "text-foreground"
                                            } cursor-default select-none relative ${selected ? "font-semibold" : "font-normal"
                                            }`}
                                    >
                                        {option.icon && (
